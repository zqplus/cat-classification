...
def detect(
    ...
    for i, (path, img, im0) in enumerate(dataloader):
        ...
        if len(pred) > 0:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls in detections:
                ...
                if cls == 15:
                	...
                	xx2=int(x2-2)
                	yy2=int(y2-2)
                	imm1=im0[yy1:yy2, xx1:xx2]
                	dim =(256,256)
                	imm1_resize=cv2.resize(imm1,dim,interpolation = cv2.INTER_AREA)
                	cv2.imwrite(save_path, imm1_resize)
        ...
        if save_images:  # Save generated image with detections
            jyd = 0#cv2.imwrite(save_path, im0)
            #cv2.imwrite(save_path, imm1)
